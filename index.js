/* ============================================================
   [VAULT] TICKET SYSTEM — VISKAS VIENAME FAILE
   Discord ticket botas + web dashboard
============================================================ */
...
try { discord = require('discord.js'); } catch (e) {
  console.error('❌ Nerasta discord.js! Paleisk: npm init -y && npm install discord.js');
  process.exit(1);
}
const {
...
function quickMessages(s) {
  const a = s.addresses || {};
  const pay = (coin, addr) =>
    '💳 **' + coin + ' apmokėjimo adresas:**\n\`\`\`' + (addr || coin + ' adresas nenustatytas (Nustatymai)') + '\`\`\`\nSiųskite tikslią sumą ir atsiųskite payslip šioje ticketėje.';
  return [
    { value: 'hi',          label: '👋 Sveiki / atsiskaitymas', desc: 'Trumpas atsakymas klientui', text: 'Sveiki! Ačiū, kad pasirinkote ' + (s.botName || '[Vault]') + '. Parašykite savo klausimą / užsakymą — atsakysime kuo greičiau. 🙏' },
    { value: 'btc',         label: '💳 BTC adresas',   desc: 'Įmeta Bitcoin apmokėjimo adresą',  text: pay('BTC', a.BTC) },
    { value: 'ltc',         label: '💳 LTC adresas',   desc: 'Įmeta Litecoin apmokėjimo adresą', text: pay('LTC', a.LTC) },
    { value: 'eth',         label: '💳 ETH adresas',   desc: 'Įmeta Ethereum apmokėjimo adresą', text: pay('ETH', a.ETH) },
    { value: 'sol',         label: '💳 SOL adresas',   desc: 'Įmeta Solana apmokėjimo adresą',   text: pay('SOL', a.SOL) },
    { value: 'nostock',     label: '⛔ Prekės neturime', desc: 'Atsiprašymas + laukiame papildymo', text: '⛔ **Atsiprašome — šiuo metu prekės neturime.**\nLaukiame papildymo. Informuosime iš karto, kai tik prekė atsiras. 🙏' },
    { value: 'refund-done', label: '💸 Refund atliktas', desc: 'Trumpa žinutė', text: '💸 **Refund atliktas.** Pinigai grąžinti — pavedimas gali užtrukti iki 24 val.' },
    { value: 'refund-no',   label: '🚫 Refund negalimas', desc: 'Pinigai negali būti grąžinti', text: '🚫 **Refund negalimas.** Pagal taisykles pinigai šiuo atveju negali būti grąžinti.' }
  ];
}

function panelEmbed(s) {
  const n = s.botName || '[Vault]';
  return new EmbedBuilder().setColor(0x22c55e).setTitle(n + ' | Support').setDescription(
    '️ **Ticket sistema**\n\nSveiki atvykę į **' + n + '** pagalbos centrą!\n\n**Pasirink teisingą kategoriją**, kitu atveju ticket gali būti uždarytas.\n**Max ' + (s.maxActive || 2) + ' aktyvūs ticket vienu metu**\n\n> 🟢 Atsakome kuo greičiau — netaginkite administracijos.\n\n**• Kategorijos**\n' +
...
function ticketEmbed(s, user, cat, num) {
  const n = s.botName || '[Vault]';
  return new EmbedBuilder().setColor(0x22c55e).setTitle(n).setDescription(
    'Sveikas, <@' + user.id + '>!\n\nJūsų bilietas buvo sėkmingai sukurtas. Mūsų komanda jį peržiūrės kuo greičiau, todėl prašome kantrybės. Kad darbas vyktų sklandžiai, netaginkite administracijos — mes patys su jumis susisieksime, kai tik turėsime atsakymą.\n\nAčiū už supratingumą!\n\n**• Klausimas:**\nticket\n\n**• Kategorija:**\n' + cat.emoji + ' ' + cat.label + '\n\n**• Ticket ID:**\n#' + num + '\n\n' + n + ' • Support'
  ).setTimestamp();
}
...
/* ---------------- Ticket kūrimas / uždarymas / panelė ---------------- */
async function createTicket(i, cat) {
  const s = dbGet();
  const max = Number(s.maxActive) || 2;
  const open = Object.values(s.tickets).filter(t => t.userId === i.user.id && t.status === 'open');
  if (open.length >= max) return i.reply({ content: '❌ **Max ' + max + ' aktyvūs ticket vienu metu.** Palauk, kol vienas bus uždarytas.', ephemeral: true });
  ...
  s.tickets[ch.id] = { id: num, userId: i.user.id, category: cat.id, status: 'open', claimedBy: null, createdAt: Date.now() };
  saveDb();
  const msg = await ch.send({ embeds: [ticketEmbed(s, i.user, cat, num)], components: staffRows(s) });
  try { await msg.pin(); } catch (e) {}
  await i.editReply({ content: '✅ Ticket **#' + num + '** sukurtas: <#' + ch.id + '>' });
...
async function closeTicket(i) {
  ...
  await i.editReply({ content: '🔒 **Ticket uždarytas.** Kanalas užrakintas ir perkeltas į uždarytų kategoriją.' });
...
async function sendPanel(client, channelId) {
  ...
  const msg = await ch.send({ embeds: [panelEmbed(s)], components: panelRows() });
  ...
  client.on('interactionCreate', async (i) => {
    try {
      ...
      if (i.isButton() && i.customId.startsWith('panel:')) {
        const cat = CATEGORIES.find(c => c.id === i.customId.split(':')[1]);
        if (cat) return await createTicket(i, cat);
      }
      if (i.isButton() && i.customId.startsWith('t:')) {
        if (!isStaff(i)) return i.reply({ content: '❌ Tik staff gali naudoti šiuos mygtukus.', ephemeral: true });
        const t = s.tickets[i.channel.id];
        switch (i.customId) {
          case 't:claim':
            if (t) { t.claimedBy = i.user.id; saveDb(); }
            return i.reply({ content: '🙋 Ticketą apsiėmė <@' + i.user.id + '>' });
          case 't:paid':
            s.stats.apmoketi = (Number(s.stats.apmoketi) || 0) + 1; saveDb();
            return i.reply({ content: '💰 **Apmokėta** — užsakymas patvirtintas, ruošiama prekė.' });
          case 't:send':
            return i.reply({ content: '📦 **Prekė išsiųsta** — patikrinkite DM / šį kanalą.' });
          case 't:done':
            s.stats.pristatyti = (Number(s.stats.pristatyti) || 0) + 1; saveDb();
            return i.reply({ content: '✅ **Atlikta.** Ačiū, kad naudojatės ' + (s.botName || '[Vault]') + '!' });
          case 't:remind':
            return i.reply({ content: '⏰ **Priminimas:** ' + (t ? '<@' + t.userId + '>' : '') + ' prašome kuo greičiau atsakyti į staff žinutę arba atlikti apmokėjimą.' });
          case 't:rename': {
          ...
          case 't:info': {
            const e = new EmbedBuilder().setColor(0x22c55e).setTitle('👤 Ticket info').addFields(
              { name: 'Ticket ID', value: t ? '#' + t.id : '—', inline: true },
              { name: 'Klientas', value: t ? '<@' + t.userId + '>' : '—', inline: true },
              { name: 'Kategorija', value: t ? ((CATEGORIES.find(c => c.id === t.category) || {}).label || t.category) : '—', inline: true },
