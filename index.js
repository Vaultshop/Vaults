/* ============================================================
   [VAULT] TICKET SYSTEM v2 — VISKAS VIENAME FAILE
   Gražūs embed'ai + mentions + užklausos rašymas
============================================================ */
...
function baseEmbed(s, guild, color) {
  ...
  const icon = guild && guild.iconURL ? guild.iconURL() : null;
  const e = new EmbedBuilder()
    .setColor(color || 0x22c55e)
    .setAuthor({ name: n + ' │ Support', iconURL: icon || undefined })
    .setFooter({ text: n + ' • Support', iconURL: icon || undefined })
    .setTimestamp();
  return e;
}
...
function panelEmbed(s, guild) {
  const n = s.botName || '[Vault]';
  const icon = guild && guild.iconURL ? guild.iconURL() : null;
  const e = baseEmbed(s, guild, 0x22c55e)
    .setDescription(
      '🎟️ **│ TICKET SISTEMA**\n\n' +
...
/* ---------------- Ticket kūrimas (su užklausos rašymu) ---------------- */
...
function registerHandlers(client) {
  client.on('interactionCreate', async (i) => {
    try {
      const s = dbGet();
      /* Panelės mygtukas → modal su užklausos rašymu */
      ...
      if (i.isButton() && i.customId.startsWith('t:')) {
        if (!isStaff(i)) return i.reply({ embeds: [staffEmbed(s, i.guild, '❌', 'Tik staff', 'Šiuos mygtukus gali naudoti tik komanda.', 0xef4444)], ephemeral: true });
        ...
        switch (i.customId) {
          ...
          case 't:send':
            return i.reply({ embeds: [staffEmbed(s, i.guild, '📦', 'Prekė išsiųsta', 'Prekė perduota klientui 🎁\nPatikrinkite DM / šį kanalą.')] });
          case 't:done':
      ...
      if (i.isStringSelectMenu() && i.customId === 't:quick') {
        if (!isStaff(i)) return i.reply({ embeds: [staffEmbed(s, i.guild, '❌', 'Tik staff', 'Šis meniu tik komandai.', 0xef4444)], ephemeral: true });
